...
parser.add_argument('--ppi_path', default=prefix+'/protein_info/protein.actions.SHS27k.STRING.pro2.txt', type=str,
                    help="ppi path")
parser.add_argument('--pseq_path', default=prefix+'/protein_info/protein.SHS27k.sequences.dictionary.pro3.tsv', type=str,
                    help="protein sequence path")
parser.add_argument('--vec_path', default=prefix+'/protein_info/vec5_CTC.txt', type=str,
                    help='protein sequence vector path')
parser.add_argument('--p_feat_matrix', default=prefix+'/protein_info/x_list.pt', type=str,
                    help="protein feature matrix")
parser.add_argument('--p_adj_matrix', default=prefix+'/protein_info/edge_list_12.npy', type=str,
                    help="protein adjacency matrix")
parser.add_argument('--split', default='random', type=str,
                    help='split method, random, bfs or dfs')
parser.add_argument('--save_path', default=prefix+'/result_save', type=str,
                    help="save folder")
...
def train(batch, p_x_all, p_edge_all, model, graph, ppi_list, loss_fn, optimizer, device,
    ...
    for epoch in range(epochs):
        ...
        steps = math.ceil(len(graph.train_mask) / batch_size)  # 要跑多少个step
        ...
        for step in range(steps):
            ...
            if got:
                ...
                label = graph.edge_attr_got[train_edge_id]  # HIGH-PPI改了p_x_all和p_edge_all，估计就是过滤后的
            else:
...
def main():
    ...
    ppi_data.split_dataset(train_valid_index_path=prefix+'/train_val_split_data/train_val_split_{}.json'.format(str(seed_num)), 
                           random_new=True, mode=args.split)  # 这里生成随机种子对应的数据集划分文件
    graph = ppi_data.data
...
# -------------------------------------------------------------
    ...
    save_path = os.path.join(save_path, "gnn_training_seed_{}".format(str(seed_num)))  # 某个特定随机数种子下的实验结果文件夹
    result_file_path = os.path.join(save_path, "valid_results.txt")  # 训练日志
    config_path = os.path.join(save_path, "config.txt")  # 这个好像没有用到
    os.mkdir(save_path)
